...
{$ENDIF}
  ...
  public
    ...
    function getCoords(prevCoordX: Integer; var coordX: Integer): Boolean;
...
begin
  // check for error
  if (level = LOGSEVERITY_ERROR) or (level = LOGSEVERITY_FATAL) then begin
    outputdebugstring(pchar(inttostr(line)+' error from consolemessage ' + message));
    haveClientError := True;
  end;
end;
...
function TForm1.getCoords(prevCoordX: Integer; var coordX: Integer): Boolean;
var
  coordsBMP: TBitmap;
  Match: TMatch;
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            coordX := prevCoordX; // default value
...
begin
  ...
  FTask := ttask.run(
    procedure
    const
      sleepDelay = 90; // no funciona con menos de 90  ,
    ...
    var
      ...
      prevCoordX: Integer;
      sCoordK, sCoordX, sCoordY: String;
      searchMerc: TProc;
      // updateCoords: TProc;
      panRight: TProc;
      panDown: TProc;
    ...
    begin
       ...
       var err1,err2:boolean;
        begin
         ...
         TThread.Synchronize(nil,
          ...
          result:=false;
          if (err1 or err2) then result:=true;
        end;
      // ---------------
      searchMerc := procedure
        ...
        begin
          ...
          TThread.Synchronize(nil,
            procedure
            begin
              if (not haveClientError) and (not haveError) then
                Exit;
        ...
        begin
          ...
          begin
            ...
            TThread.Synchronize(nil,
              procedure
              begin
                MouseMove([], 644, 600); // 600...210
                MouseDown(mbLeft, [], 644, 600);
              end);
      ...
      for i := 0 to memoKingdoms.lines.Count - 1 do
      begin
        ...
        TThread.Synchronize(nil,
          procedure
          begin
            // search icon button  90, 543
            MouseMove([], 90, 543);
            MouseDown(mbLeft, [], 90, 543);
            sleep(10);
            MouseUp(mbLeft, [], 90, 543);
          end);
        ...
        begin
          ...
          TThread.Synchronize(nil,
            ...
            begin
              // // kindom 419, 360
              MouseMove([], 419, 360);
              MouseDown(mbLeft, [], 419, 360);
              sleep(10);
              MouseUp(mbLeft, [], 419, 360);
            end);
          ...
          TThread.Synchronize(nil,
            ...
            begin
              // // x 512, 360
              MouseMove([], 512, 360);
              MouseDown(mbLeft, [], 512, 360);
              sleep(10);
              MouseUp(mbLeft, [], 512, 360);
            end);
          ...
          TThread.Synchronize(nil,
            ...
            begin
              // y 615, 360
              MouseMove([], 615, 360);
              MouseDown(mbLeft, [], 615, 360);
              sleep(10);
              MouseUp(mbLeft, [], 615, 360);
            end);
          ...
          TThread.Synchronize(nil,
            ...
            begin
              // go button 509,402
              MouseMove([], 509, 402);
              MouseDown(mbLeft, [], 509, 402);
              sleep(10);
              MouseUp(mbLeft, [], 509, 402);
            end);
          ...
          begin
            ...
            begin
              ...
              TThread.Synchronize(nil,
                ...
                begin
                  ...
                  coordXOK := getCoords(prevCoordX, coordX);
                end);

              // if TryStrToInt(sCoordX, coordX) then
              if coordXOK then
              begin
                prevCoordX := coordX;
                // outputdebugstring(pchar('derecha ' + sCoordX + ' : ' +
                // inttostr(coordX)));
                if (coordX > 950) then
                  break;
              ...
              begin
                // ocr failed, based on prev data, we continue on the same direction
                if (prevCoordX > 0) and (prevCoordX < 900) then
                // prevcoord was ok,
                begin
                  panRight();
                  searchMerc;
                ...
                begin
                  // fallo 2 veces
                  // outputdebugstring(pchar('no pudo convertir coordX ' + sCoordX
                  // + ' : ' + inttostr(coordX)));
                  TThread.Synchronize(nil,
                    procedure
                    begin
                      Memo1.lines.add('fail ' + sCoordX);
                    end);
                  break;
                end;
            ...
            begin
              ...
              begin
                // ocr failed, based on prev data, we continue on the same direction
                if (prevCoordX > 0) and (prevCoordX > 50) then
                // prevcoord was ok,
                begin
                  panLeft();
                  searchMerc;
...
{$ENDIF}
  ...
  ScreenCoords := Mouse.CursorPos;
  Pt.X := X;
  Pt.Y := Y;
  LocalPt := Panel1.ScreenToclient(Pt);
  ...
  // outputdebugstring(pchar(Format('Screen Coords -> X: %d, Y: %d',
  // [ScreenCoords.X, ScreenCoords.Y])));
  // outputdebugstring(pchar('mouse down ' + inttostr(X) + ',' + inttostr(Y)));
  // outputdebugstring(pchar('mouse down ' + inttostr(TempEvent.X) + ',' +
  // inttostr(TempEvent.Y)));
  // outputdebugstring(pchar('mouse down ' + inttostr(LocalPt.X) + ',' +
  // inttostr(LocalPt.Y)));
end;
